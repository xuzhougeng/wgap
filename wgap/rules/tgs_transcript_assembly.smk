...
rule build_mm2_index:
    ...
    shell:"""
        minimapt2 -t {threads} {params.opts} -I 100G -d {output} {input}
    """
...
rule pb_map:
    ...
    output: "rna-seq/tgs/02-read-align/{sample}_pb.bam"
    params: 
...
rule pb_map:
    input:
        index = rules.build_mm2_index.output
        fastq = rules.tgs_pre_processing.output
    output: "rna-seq/tgs/02-read-align/{sample}_ont.bam"
    params: 
...
rule pb_stringtie_merge:
    ...
    params:
        label = lambda wildcards: "{tissue}_pb".format(wildcards.tissue)
...
rule ont_stringtie_merge:
    ...
    params:
        label = lambda wildcards: "{tissue}_ont".format(wildcards.tissue)
    shell: "stringtie --merge -l {params.label}  -o {output} {input}"
